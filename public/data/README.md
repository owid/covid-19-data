...
> ⚠️ Our previous source for confirmed cases and deaths, the European Centre for Disease Prevention and Control (ECDC), [announced in November](https://www.ecdc.europa.eu/en/publications-data/download-todays-data-geographic-distribution-covid-19-cases-worldwide) that it would switch from a daily to a weekly reporting schedule from December. _Our World in Data_ therefore had to transition away from the ECDC as a source to continue to provide daily updates of confirmed cases and deaths. **Since November 30, the data for confirmed cases and deaths presented on [ourworldindata.org](https://ourworldindata.org/coronavirus-data-explorer) and available in our GitHub repository is sourced from the daily dataset provided by [Johns Hopkins University](https://github.com/CSSEGISandData/COVID-19).** The data last sourced from the ECDC remains available as an archive in the [`ecdc`](https://github.com/owid/covid-19-data/tree/master/public/data/ecdc) folder. The format (variable names and types) of our complete COVID-19 dataset remains the same. If you have any scripts or automated data processes, they should continue to work, but the source data is different; please make sure you update your source credits accordingly.
...
- On 9 December 2020, we changed the names of three countries in our files to better match their official names. `Czech Republic` has become `Czechia`, `Macedonia` has become `North Macedonia`, and `Swaziland` has become `Eswatini`.
